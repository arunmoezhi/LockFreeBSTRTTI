...
public class TestLockFreeBSTRTTI implements Runnable
{
	int threadId;
	...
	public TestLockFreeBSTRTTI(int threadId)
	{
	...
	final void generateInput(int threadId)
	{
		...
		if(threadId == 0)
		{
			...
			{
				obj.insert(rd.nextInt(keyRange)+1, rd.nextInt(keyRange)+1, threadId);
			}
		}
		else
		{
			...
			for(int i=0;i<iterations;i++)
			{
				...
				{
					obj.delete(rd.nextInt(keyRange)+1, threadId);
				}

			}
			end = System.currentTimeMillis();
			TestLockFreeBSTRTTI.timeArray[threadId-1] = end-start;
		}
	}
	...
	public static void main(String[] args)
	{
		...
		TestLockFreeBSTRTTI.timeArray = new long[NUM_OF_THREADS];
		try
		{
			obj = new LockFreeBSTRTTI();
			obj.createHeadNodes();
			Thread[] arrayOfThreads = new Thread[NUM_OF_THREADS+1];

			arrayOfThreads[0] = new Thread(new TestLockFreeBSTRTTI(0)); //just inserts - initial array
			...
			{
				arrayOfThreads[i] = new Thread(  new TestLockFreeBSTRTTI(i));
			...
			obj.nodeCount(LockFreeBSTRTTI.grandParentHead);
			long totalTime=0;
			float avgTime=0.0f;
			for(int i=0;i<NUM_OF_THREADS;i++)
			{
				totalTime += TestLockFreeBSTRTTI.timeArray[i];
			}
			
			avgTime = (totalTime * 1.0f) /(NUM_OF_THREADS * 1.0f);
			Mops = (long)iterations * NUM_OF_THREADS * NUM_OF_THREADS * 1.0d /(totalTime * 1000.0d);
			
			System.out.print("k" + args[3] + ";" + args[1] + ";" + args[0] + ";" + LockFreeBSTRTTI.nodeCount);
